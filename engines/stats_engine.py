...
def compute_dashboard_stats(
    ...
    Retorna: {total_athletes, active_athletes, pending_registration,
              total_seasons, active_seasons, closed_seasons,
              active_season_id, active_season_name,
              total_results, pending_results, confirmed_results,
              total_rounds, active_rounds}
    """
    ...
    return {
        ...
        "pending_results":      sum(1 for r in results if r.get("status") == "pending"),
        "confirmed_results":    sum(1 for r in results if r.get("status") == "confirmed"),
        "total_rounds":         len(rounds),
        "active_rounds":        sum(1 for r in rounds if r.get("status") == "active"),
    }
